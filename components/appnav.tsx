...
import { Box, IconButton, Button, Drawer, DrawerBody, DrawerOverlay, DrawerContent, DrawerCloseButton, useDisclosure, Flex } from "@chakra-ui/react";
...
const Nav: NextPage = () => {
  ...
  return (
    <>
   <Box 
    className="relative w-full pr-10 pl-2 top-0 left-0 bg-linen backdrop-filter[blur(26px)] box-border lg:w-90rem flex flex-row py-4 items-center justify-between text-left text-sm text-light-green font-karla border-b border-solid border-darkslategray-100 space-x-4 lg:space-x-0"
  >
    <Box className="flex flex-row items-center justify-start gap-1 lg:gap-4">
      ...
      <div className="flex flex-col items-start justify-center gap-2">
        ...
        <Box className="hidden lg:flex flex-row space-x-0">
          ...
          <Button 
            className="rounded-md bg-transparent border border-solid border-white flex flex-row py-3 px-6 items-center justify-center text-white"
          >
            Connect Wallet
          </Button>
        </Box>
